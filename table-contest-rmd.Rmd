...
We are wanting to control something to do with *columns* so we'll use a `cols_*()` function. This family of functions can move columns around, merge them, hide them and - important right now - rename them. The syntax of `cols_label()` is almost the inverse of `dplyr::rename()` - instead of "new = old", "old = new" is provided. The new name should also be given as a string.
...
```{r timeplot}
timePlot(data, pollutant = "conc", type = "species", 
         y.relation = "free", key = F, avg.time = "week")
```
<br>
...
our_table
save(our_table, openair_image, data_plots, file = "~/Downloads/our_table.rda")
#save.image(file = "~/Downloads/our_table.rda")
```
